...
struct ContentView1: View {
    ...
    // JSON files
    let Checkout01: Checkout = Bundle().decode("CheckoutOption1.json")
    let Checkout02: Checkout = Bundle().decode("CheckoutOption2.json")
    let Checkout03: Checkout = Bundle().decode("CheckoutOption3.json")
                ...
                VStack{
                    ...
                    Section{
                        VStack{
                            Spacer()
                            Section{
                                OutText(text: "\(Checkout01.checkoutOptions.first?["\(thisOut)"]?.out ?? "")")
                            } header: {
                                OptionText(text: "     Option 1:")
                            }
                            Spacer()
                            Section{
                                OutText(text: "\(Checkout02.checkoutOptions.first?["\(thisOut)"]?.out ?? "")")
                            } header: {
                                OptionText(text: "     Option 2:")
                            }
                            Spacer()
                            Section{
                                OutText(text: "\(Checkout03.checkoutOptions.first?["\(thisOut)"]?.out ?? "")")
                            } header: {
